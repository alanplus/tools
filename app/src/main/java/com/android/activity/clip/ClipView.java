...
public class ClipView extends RelativeLayout implements OnTouchListener {
    ...
    private String saveHeadBitmap(Bitmap bitmap) {
        String destPath = getBitmapDestPath();
        BitmapUtils.save(bitmap, destPath);
        return destPath;
    }

    private String getBitmapDestPath() {
        String path = "/data/data/" + getContext().getPackageName() + "/temp";
        File fileDir = new File(path);
        if (!fileDir.exists()) {
            fileDir.mkdirs();
        }
        return path + "head.png";
    }
